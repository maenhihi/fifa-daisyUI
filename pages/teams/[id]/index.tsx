    ...
    <section id="TeamDetails" className='h-screen'>
      ...
      <h2 className='p-4 text-indigo-900 flex justify-around items-center text-3xl font-bold'>Here's how {team.name} has been going so far</h2>
        ...
        <div className="flex flex-wrap justify-around items-center gap-4">
          {recentMatches.map((array: { id: Key | null | undefined; date: string; opponent: string; result: string; }) => (
            <IndividualTeamMatch
              key={array.id}
              date={array.date}
        ...
        <div className='flex flex-wrap justify-center items-center gap-4'>
          {teamPlayers.map((array: { id: Key | null | undefined; name: string; position: string; age: number; club: string; }) => (
            <Individualplayers
              key={array.id}
              name={array.name}
